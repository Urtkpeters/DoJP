...
function buildLevel()
{
    ...
    objLevel.objMusic = objSounds[objLevelLibrary['level' + objGame.nextLevel].levelMusic];

    objPlayer.batteryPower = 0;

    $.each(objLevelLibrary['level' + objGame.nextLevel].levelEntities, function()
    {
        ...
        for(var key2 in objLevel.objBullets)
        {
            if(objLevel.objEntities[key].type == 'monster' && checkCollision(objLevel.objBullets[key2], objLevel.objEntities[key]))
            {
                ...
                if(objLevel.objEntities[key].updateHP(objPlayer.IDE[objPlayer.IDE.active].damage) <= 0)
                {
                    ...
                    console.log(objLevel.objEntities[key]);
    ...
    if(objGame.gamePaused == false)
    {
        ...
        for(var key in objLevel.objEntities)
        {
            ...
            else if(objLevel.objEntities[key].attackType == 'special' && objLevel.objEntities[key].monsterFire() == false)
            {
                ...
                objLevel.objEntities['entity' + objLevel.intEntityCounter] = buildEntity(aryAttackOrder[objLevel.objEntities[key].attackNumber] + 'TombstoneAttack', aryAttackOrder[objLevel.objEntities[key].attackNumber] + 'Tombstone', 'stationary', 0, 'temp');
