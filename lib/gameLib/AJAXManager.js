...
function saveGame()
{
    ...
    if(objGame.loggedIn)
    {
        ...
        if(typeof(sessionId) !== 'undefined' && sessionId > 0)
        {
            $.ajax
            ({
                url: '/event/saveGame?sessionId=' + sessionId + '&level=' + objPlayer.nextLevel + '&earnings=' + objPlayer.earnings + '&score=' + objPlayer.score + '&pto=' + objPlayer.pto + '&windate=' + objPlayer.windate + '&nespresso=' + objPlayer.nespresso + '&activeide=' + objPlayer.IDE.active + '&notepad=' + objPlayer.IDE.notepad.level + '&notepadplusplus=' + objPlayer.IDE.notepadplusplus.level + '&far=' + objPlayer.IDE.far.level + '&eclipse=' + objPlayer.IDE.eclipse.level + '&dreamweaver=' + objPlayer.IDE.dreamweaver.level + '&mulestudio=' + objPlayer.IDE.muleStudio.level + '&intelliJ=' + objPlayer.IDE.intelliJ.level + '&netbeans=' + objPlayer.IDE.netbeans.level + '&purchasedPTO=' + objPlayer.purchasedPTO,
                cache: false,
                dataType: 'json',
                success: function (responseData)
                {
